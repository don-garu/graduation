...
//		printf("Edge [%d] - %d ~ %d(%f)\n", i, a, b, w);
	...
	while (pq.size(&pq)){
		...
		int u = p.first;
		float cost = p.third;
		float f_score = p.second;
		int len = adj[u].size;
		if (cost > dist[u]) continue;
		for (int i = 0; i<len; i++){
			pair* v = adj[u].accessByIndex(&adj[u], i);
			float temp_cost = v->second + cost;
			if (temp_cost < dist[v->first]){
				dist[v->first] = temp_cost ;
                p.first = v->first;
				p.second = dist[p.first] + cal_h(location[v->first], location[end]);
				p.third = dist[p.first];
                pq.push(&pq, p);
			}
		}
	}

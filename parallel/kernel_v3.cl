...
__kernel void INITIAL(__global float *f_score, __global float *weight, __global int *open_list, __global int *prev, int N){
...
inline void atomic_test(volatile __global float *g_f_score, float my_f_score, volatile __global float *g_weight, float my_weight, volatile __global int *g_prev, int my_prev){
    float cur;
    while (my_f_score < (cur = *g_f_score)){
        my_f_score = atomic_xchg(g_f_score, my_f_score);
        my_weight = atomic_xchg(g_weight, my_weight);
...
__kernel void EDGE_RELAXING(__global node *nodes, __global float *f_score, __global float *weight, __global int *open_list, __global int *prev, int N, int target){
    int id = get_global_id(0);
    int tmp;
    if (id < N){
        if (open_list[id] == 1){ // 1 means queued, 0 means not queued
            ...
            for (tmp = 0; tmp < BRANCH; tmp++){
                ...
                float next_weight = my_weight + my_node.adj_node[tmp].weight;
                float f_next = next_weight + cal_h(nodes[next], nodes[target]);// + sqrt(pow((float)(nodes[next].x - nodes[target].x), (float)2) + pow((float)(nodes[next].y - nodes[target].y), (float)2));
                if (next != -1 && f_next < weight[next]){
                    // 발생 가능 문제 : IF 문을 통과한 뒤 context-switching 이 발생해서 원치 않은 결과가 반영될 수 있음?
                    atomic_test(&f_score[next], f_next, &weight[next], next_weight, &prev[next], id);

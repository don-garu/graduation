...
int main(int argc, char *argv[]){
 ...
 //   else if (n <= 10000)
  //      l_size = 50;
    ...
    f_score = (float*)calloc(sizeof(float), n);
...
/*    for (int i = 0; i<n; i++){
    ...
    cl_mem buf_node, buf_visited, buf_weight, buf_f_score, buf_prev, buf_gopen, buf_open_list;
    ...
    CHECK_ERROR(err);
    buf_f_score = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(float), NULL, &err);
    CHECK_ERROR(err);
    ...
    CHECK_ERROR(err);
    err = clEnqueueWriteBuffer(queue, buf_f_score, CL_FALSE, 0, n * sizeof(float), f_score, 0, NULL, NULL);
    CHECK_ERROR(err);
    ...
    // STEP 1
    // INITIAL
    err = clSetKernelArg(kernel[0], 0, sizeof(cl_mem), &buf_f_score);
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel[0], 1, sizeof(cl_mem), &buf_weight);
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel[0], 2, sizeof(cl_mem), &buf_open_list);
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel[0], 3, sizeof(cl_mem), &buf_prev);
    CHECK_ERROR(err);
    ...
    CHECK_ERROR(err);
    err = clEnqueueReadBuffer(queue, buf_f_score, CL_TRUE, 0, sizeof(float) * n, f_score, 0, NULL, NULL);
    CHECK_ERROR(err);
    ...
    CHECK_ERROR(err);
    err = clEnqueueWriteBuffer(queue, buf_f_score, CL_FALSE, 0, sizeof(float) * n, f_score, 0, NULL, NULL);
    CHECK_ERROR(err);
...
/*      printf("weight\n");
        ...
        CHECK_ERROR(err);
        err = clSetKernelArg(kernel[2], 1, sizeof(cl_mem), &buf_f_score);
        CHECK_ERROR(err);
        err = clSetKernelArg(kernel[2], 2, sizeof(cl_mem), &buf_weight);
        CHECK_ERROR(err);
        err = clSetKernelArg(kernel[2], 3, sizeof(cl_mem), &buf_open_list);
        CHECK_ERROR(err);
        err = clSetKernelArg(kernel[2], 4, sizeof(cl_mem), &buf_prev);
        CHECK_ERROR(err);
        err = clSetKernelArg(kernel[2], 5, sizeof(int), &n);
        CHECK_ERROR(err);
        err = clSetKernelArg(kernel[2], 6, sizeof(int), &end);
        CHECK_ERROR(err);
    ...
    err = clEnqueueReadBuffer(queue, buf_weight, CL_FALSE, 0, sizeof(float) * n, weight, 0, NULL, NULL);
    CHECK_ERROR(err);
    err = clEnqueueReadBuffer(queue, buf_f_score, CL_FALSE, 0, sizeof(float) * n, f_score, 0, NULL, NULL);
    CHECK_ERROR(err);
    ...
    //printf("Total Weight : %f, %f\n", weight[prev[end]], f_score[prev[end]]);
    int t = end;
    printf("%d", t);
    while (prev[t] != -1){
        printf(" - %d", prev[t]);
